...
export function SignalsTable({ initialData }: SignalsTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("signals").delete().eq("id", deleteId)
        ...
        <Table>
            <TableHeader className="bg-muted/20">
              <TableRow className="hover:bg-transparent border-b border-border">
                <TableHead className="w-[180px] h-10 text-[10px] uppercase tracking-widest font-semibold text-muted-foreground pl-6">Asset</TableHead>
                <TableHead className="h-10 text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Direction</TableHead>
                <TableHead className="h-10 text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Entry Price</TableHead>
                <TableHead className="h-10 text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Status</TableHead>
                <TableHead className="h-10 text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">Result</TableHead>
            ...
            <TableBody>
              {data.length === 0 ? (
                ...
                data.map((signal) => (
                  <TableRow key={signal.id} className="h-14 border-border/40 hover:bg-muted/30 transition-colors">
                    <TableCell className="pl-6">
                        <div className="flex flex-col gap-0.5">
                            <span className="font-mono text-sm font-medium">{signal.asset}</span>
                    ...
                    <TableCell>
                      <span
                        className={cn("inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium uppercase border", statusColors[signal.status.toLowerCase()] || statusColors.draft)}
                      >
                        {signal.status}
                      </span>
                    </TableCell>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the signal and remove it from the dashboard.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
