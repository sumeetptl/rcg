  ...
  useEffect(() => {
    const fetchSignal = async () => {
      ...
      setFormData({
        ...
        take_profit_1: data.take_profit_1?.toString() || "",
        take_profit_2: data.take_profit_2?.toString() || "",
        take_profit_3: data.take_profit_3?.toString() || "",
        analysis: data.analysis || "",
        status: data.status,
        result_pnl: data.result_pnl?.toString() || "",
      })
      setIsFetching(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        result_pnl: formData.result_pnl ? parseFloat(formData.result_pnl) : null,
      })
      .eq("id", signalId)
    ...
    <div className="mx-auto max-w-4xl p-6 lg:p-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
         <div className="flex flex-col gap-1">
            ...
            <h1 className="font-serif text-3xl font-semibold tracking-tight">Edit Signal</h1>
         </div>
        ...
        <FormSection title="Signal Overview" description="Core details about the asset and trade direction.">
            <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                 ...
                 <div className="space-y-2">
                    <Label htmlFor="result_pnl">Result P&L (%)</Label>
                    <Input
                        id="result_pnl"
                        type="number"
                        step="any"
                        placeholder="Optional"
                        value={formData.result_pnl}
                        onChange={(e) => setFormData({ ...formData, result_pnl: e.target.value })}
                        className="font-mono"
                    />
                </div>
            </div>
